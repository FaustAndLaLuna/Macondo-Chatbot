...
@retry(wait=wait_random(min=60, max=63), stop=stop_after_attempt(6))
def get_ChatGPT_plant_datum(plant_name:str) -> str:
...
@retry(wait=wait_random(min=1, max=4), stop=stop_after_attempt(6))
def get_ChatGPT_response(question:str, history_map: List[Dict]) -> str:
    """Gets ChatGPT percentage of how good the candidate fit is as a percentage."""
    query = [{'role' : 'system', 'content' : f'Tú eres un vendedor de plantas llamado Silvestre. Trabajas para macondo. Te encanta la jardinería.'}]
    ...
    query.append({'role': 'user', 'content' : f'Ignora árboles. Responde en hasta 35 palabras {question}'.replace('\n', ' ').replace('  ',' ').replace('  ',' ').replace('  ',' ')})
...
def get_embedding(text: str, engine="text-embedding-ada-002") -> List[float]:
    try:
        ...
        print(f'Iterating... Time = {time.localtime()}')
...
@retry(wait=wait_random(min=60, max=63), stop=stop_after_attempt(6))
def get_embeddings_from_list(list_of_texts: List[str], engine="text-embedding-ada-002") -> List[List[float]]:
    try:
        response = []
        # replace newlines, which can negatively affect performance.
...
@retry(wait=wait_random(min=70, max=600), stop=stop_after_attempt(6))
def get_embeddings(
...
def chart_from_components(
    ...
    **kwargs,
    ...
    data = pd.DataFrame(
        {
            ...
            "label": labels if has_labels else empty_list,
    ...
    chart = px.scatter(
        ...
        x=x_title,
        y=y_title,
        color="label" if has_labels else None,
        symbol="label" if has_labels else None,
        hover_data=["string"] if has_strings else None,
        **kwargs,
